...
export default function EmpHome() {
  ...
  async function loadLog() {
    ...
    try {
      ...
      // les données déjà affichées restent visibles, on peut réessayer plus tard.
  ...
  const needCode = company?.require_daily_code !== false
  ...
  // Modal si un code est exigé ou si un secteur peut être choisi, sinon pointage direct
  function startPunchFlow() {
    setCodeInput(''); setCodeError(''); setStartSector(''); setFreeInput(false)
    const nc = company?.require_daily_code !== false
    if (!nc && !company?.sectors_enabled) doPunchIn()
    else setShowCodeModal(true)
  }

  async function confirmCodeAndPunchIn() { doPunchIn() }
  ...
  async function togglePause() {
    if (!log || pauseLoading) return
    setPauseLoading(true)
    const nowIso = new Date().toISOString()
    try {
      if (isPaused) {
        const { error } = await supabase.from('time_logs').update({ pause_end: nowIso }).eq('id', log.id)
        if (error) throw error
        ...
        const { error } = await supabase.from('time_logs').update({ pause_start: nowIso }).eq('id', log.id)
        if (error) throw error
        if (company?.sectors_enabled) {
    ...
    <div className="screen">
      ...
      <div className="content">
        ...
        <div className={`punch ${isError ? 'punch-error' : isWorking||isPaused ? 'punch-live' : isDone ? 'punch-done' : 'punch-idle'}`}>
          ...
          {isDone && (
            <button className="punch-cta punch-cta-start" onClick={startPunchFlow}>
          ...
          {(isWorking||isPaused) && (
            <>
              ...
              <div className="punch-actions">
                <button className="punch-act" onClick={togglePause} disabled={pauseLoading}>
          ...
          <div className="ms" onClick={e=>e.stopPropagation()}>
            ...
            <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'16px'}}>
              <div style={{width:'46px',height:'46px',borderRadius:'50%',background:'var(--blue-bg)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                <i className={`ti ${needCode?'ti-shield-lock':'ti-map-pin'}`} style={{fontSize:'22px',color:'var(--blue)'}}/>
              </div>
              <div>
                <div style={{fontSize:'18px',fontWeight:'800'}}>{needCode?'Code de pointage':'Démarrer la journée'}</div>
                <div style={{fontSize:'13px',color:'var(--text2)'}}>{needCode?'Le code affiché sur place':'Choisissez votre secteur de départ'}</div>
              </div>
            </div>
            {needCode && (
              <input
            ...
            <button className="btn btn-p" onClick={confirmCodeAndPunchIn} disabled={checkingCode} style={{marginTop:'8px'}}>
              <i className="ti ti-check"/>{checkingCode?'…':(needCode?'Valider et pointer':'Pointer')}
            </button>
