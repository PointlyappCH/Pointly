...
export default function EmpChat() {
  ...
  async function send() {
    ...
    await supabase.from('chat_messages').insert({
      company_id: company.id, user_id: profile.id, channel, content: txt,
    })
  }
  ...
  return (
    <div className="screen">
      <div className="topbar" style={{paddingBottom:'0',flexDirection:'column',alignItems:'stretch',gap:'0'}}>
        ...
        <div className="tabs">
          {[{key:'general',label:'🌐 Général'},{key:'kitchen',label:'🍳 Cuisine'},{key:'private',label:'🔒 Privé'}].map(c=>(
            <div key={c.key} className={`tab ${channel===c.key?'active':''}`} onClick={()=>setChannel(c.key)}>{c.label}</div>
          ))}
        </div>
      </div>

      <div style={{flex:1,overflowY:'auto',padding:'14px',display:'flex',flexDirection:'column',gap:'10px'}}>
        {messages.length===0 && <div style={{textAlign:'center',fontSize:'13px',color:'var(--text3)',padding:'20px 0'}}>Aucun message pour l'instant</div>}
        {messages.map((msg,i) => {
          ...
          return (
            <div key={msg.id||i} style={{alignSelf:isMe?'flex-end':'flex-start',maxWidth:'82%'}}>
              {!isMe && <div style={{fontSize:'11px',color:'var(--text3)',marginBottom:'3px',display:'flex',alignItems:'center',gap:'5px'}}>
                <span style={{width:'16px',height:'16px',borderRadius:'50%',background:p?.color_bg||'#E6F1FB',color:p?.color_fg||'#185FA5',display:'inline-flex',alignItems:'center',justifyContent:'center',fontSize:'8px',fontWeight:'700'}}>{mkIni(p?.full_name||'')}</span>
                {p?.full_name?.split(' ')[0]}{p?.role==='admin'?' (Patron)':''}
              </div>}
